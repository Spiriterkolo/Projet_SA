...
class SBI_Dataset(Dataset):
	def __init__(self,phase='train',image_size=224,n_frames=8):
		...
		path_lm='/landmarks/'
		label_list=[label_list[i] for i in range(len(image_list)) if os.path.isfile(image_list[i].replace('\\','/').replace('/frames/','/landmarks/').replace('.png','.npy')) and os.path.isfile(image_list[i].replace('\\','/').replace('/frames/','/retina/').replace('.png','.npy'))]
		image_list=[image_list[i] for i in range(len(image_list)) if os.path.isfile(image_list[i].replace('\\','/').replace('/frames/','/landmarks/').replace('.png','.npy')) and os.path.isfile(image_list[i].replace('\\','/').replace('/frames/','/retina/').replace('.png','.npy'))]
